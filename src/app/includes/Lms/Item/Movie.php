...
class Lms_Item_Movie extends Lms_Item_Abstract_Serialized
{
    ...
    public static function createFromInfo($info, $files, $quality, $translation)
    {
        ...
        if (isset($info['imdb_id']) || $info['rating_imdb_value']) {
            $item = Lms_Item::create('Rating');
            $item->setSystem('imdb');
        ...
        if (isset($info['kinopoisk_id']) || $info['rating_kinopoisk_value']) {
            $item = Lms_Item::create('Rating');
    ...
    public static function updateLocalRating($movieId)
    {
        ...
        $rating = Lms_Item_Rating::getBayes($ratings, Lms_Application::getConfig('minratingcount'));
        $db->query(
    ...
    {
        foreach (Lms_Application::getConfig('rootdir') as $rootdir) {
    ...
    public function getFilesAsArray2()
    {
        ...
        foreach ($items as $item) {
            ...
            if (Lms_Application::getConfig('download', 'license')) {
                $v = Lms_Application::getLeechProtectionCode(array($movieId, $item->getId(), $user->getId()));
                $links['license'] = "pl.php?player=ftp&uid=" . $user->getId() . "&movieid=$movieId&fileid=" . $item->getId() . "&v=$v";
            } else {
                $links['download'] = str_replace(Lms_Application::getConfig('source'), Lms_Application::getConfig('ftp'), $path);
            ...
            if ($item->getTthHash()) {
                $links['dcpp'] = "magnet:?xt=urn:tree:tiger:" . $item->getTthHash() . "&xl=" . $item->getSize() . "&dn=" . rawurlencode(basename($path));
            }
