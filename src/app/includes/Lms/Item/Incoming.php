...
class Lms_Item_Incoming extends Lms_Item_Abstract_Serialized
{
    ...
    */
    ...
    public function mergeParsedInfo($data, $engine, $forceMerge = false)
    {
        ...
        if ($forceMerge
            || ((isset($currentInfo['year']) && isset($data['year']) && $currentInfo['year']==$data['year'])
                && (
                    (isset($currentInfo['name']) && isset($data['name']) && $currentInfo['name']==$data['name'])
                    || (isset($currentInfo['international_name']) && isset($data['international_name']) && trim($currentInfo['international_name'], ' "\'')==trim($data['international_name'], ' "\''))
                ))
        ) {
            //merge
            foreach ($data as $field => $value) {
                $oldWeight = isset($currentInfo['weights'][$field])? $currentInfo['weights'][$field] : 1;
                $newWeight = $this->getFieldWeight($engine, $field);
                if (!isset($currentInfo[$field]) 
                    || $this->compareValue($currentInfo[$field], $oldWeight, $value, $newWeight)
                ) {
                    $currentInfo[$field] = $value;
                    $currentInfo['weights'][$field] = $newWeight;
                }
            }
            $this->setInfo($currentInfo);
        } else {
            foreach ($data as $field => $value) {
                $data['weights'][$field] = $this->getFieldWeight($engine, $field);
            }
            $this->setInfo($data);
        }
        return $this;
