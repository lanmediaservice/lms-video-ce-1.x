...
include dirname(__FILE__) . "/ext_progs.php";

class Mplayer extends ExternalProgram {
	var $location_mplayer; // path for launching mplayer
	var $priority;
	...
	function Mplayer($str)
	{
		$this->location_mplayer = $str;
	}

	/**
	 * Mplayer::build_exec_srting()
	 *
	 * @return string Complete escaped command for proc_open
	 */
	function build_exec_string($params)
	{
		$str = escapeshellcmd($this->location_mplayer);
		foreach($params as $param) {
			if (is_array($param)) {
				list($option, $value) = $param;
				if (is_array($value)) {
					$suboptions = array();
					foreach($value as $subvalue) {
						if (is_array($subvalue)) {
							$suboption = $subvalue[0];
							$suboptionvalue = isset($subvalue[1]) ? $subvalue[1] : false;
						} else {
							$suboption = $subvalue;
							$suboptionvalue = false;
						}

						$suboptions[] = $suboption . (($suboptionvalue!==false)? "=" . $this->escapeshelloption($suboptionvalue): "");
					}
					$str .= " " . $option . " " . implode(":", $suboptions) . " ";
				} else $str .= " " . $option . " " . escapeshellarg($value) . " ";
			} else $str .= " " . escapeshellarg($param) . " ";
		}
		return $str;
	}
	 ...
	 * @return
	 */
	function analyze($path_to_file)
	{
		$commandStr = escapeshellcmd($this->location_mplayer);
		$commandStr .= ' -frames 1 -vo null -ao null -msglevel identify=6 ';
		$commandStr .= escapeshellarg($path_to_file);
		echo " !" . $commandStr . "! ";
		$lines = array();
		...
		$res = array();
		$res['video']['codec'] = $this->RIFFfourccLookup($info['ID_VIDEO_FORMAT']);
		if (!$res['video']['codec']) $res['video']['codec'] = $info['ID_VIDEO_CODEC'];
		$res['video']['frame_rate'] = $info['ID_VIDEO_FPS'];
		$res['video']['resolution_x'] = $info['ID_VIDEO_WIDTH'];
		$res['video']['resolution_y'] = $info['ID_VIDEO_HEIGHT'];
		$res['video']['dataformat'] = $info['ID_DEMUXER'];
		$res['playtime_seconds'] = (int) $info['ID_LENGTH'];

		$res['audio']['sample_rate'] = $info['ID_AUDIO_RATE'];
		$res['audio']['channelmode'] = $info['ID_DEMUXER'];
		$res['audio']['channels'] = $info['ID_AUDIO_NCH'];
		
		if (@$info['ID_AUDIO_NCH'] == '1') {
			$res['audio']['channelmode'] = 'mono';
		} elseif (@$info['ID_AUDIO_NCH'] == '2') {
			$res['audio']['channelmode'] = 'stereo';
		}
			
		$res['audio']['bitrate'] = $info['ID_AUDIO_BITRATE'];
		$res['audio']['codec'] = $this->RIFFwFormatTagLookup($info['ID_AUDIO_FORMAT']);
	...
	{
		if (!$str) $str = $this->location_mplayer;
...
function RIFFwFormatTagLookup($wFormatTag) {
		...
		*/
		...
		return $this->EmbeddedLookup('0x'.str_pad(strtoupper(dechex($wFormatTag)), 4, '0', STR_PAD_LEFT), $begin, __LINE__, __FILE__, 'riff-wFormatTag');
	...
	function RIFFfourccLookup($fourcc) {
		...
		*/
		...
		return $this->EmbeddedLookup($fourcc, $begin, __LINE__, __FILE__, 'riff-fourcc');
	}
