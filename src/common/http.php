 ...
 * LMS Library
 * 
 * @version $Id: http.php 309 2009-01-17 18:54:52Z macondos $
 * @package HttpClient
 */
...
***************************************************************************************************/
    ...
    class http {
        ...
        function http( $http_version = HTTP_V10, $keep_alive = false, $auth = false ) {
            ...
            $this->user_agent    = 'Mozilla/5.0 (Windows; U; Windows NT 5.1; ru; rv:1.9.0.5) Gecko/2008120122 Firefox/3.0.5';
            $this->host            = '';
            $this->port            = 80;
            $this->errstr        = '';
        ...
        function head( $uri ) {
            ...
            if ($this->_use_proxy) {
                ...
                if (substr($uri,0,7)!='http://') $uri = 'http://' . $this->host . ':' . $this->port . $uri;
            } else {
        ...
        function get( $uri, $follow_redirects = true, $referer = '', $returnRawResponse = false, $gzipEnable = false) {
            $this->uri = $uri;
            ...
            $this->_request->set_header( 'User-Agent', $this->user_agent );
            $this->_request->set_header( 'Accept', '*/*' );
            $this->_request->set_header( 'Accept-Language', 'ru' );
            if ($gzipEnable) {
                $this->_request->set_header( 'Accept-Encoding', 'gzip, deflate' );
            }
            if ($referer) $this->_request->set_header( 'Referer', $referer );
            $this->_request->set_header( 'Cookie', $http_cookie );
        ...
        function _connect( ) {
            ...
            if (!$this->_use_proxy)
                $this->_socket = fsockopen( $this->host, $this->port, $errno, $errstr, $this->connection_timeout );
            else
                $this->_socket = fsockopen( $this->_proxy_host, $this->_proxy_port, $errno, $errstr, $this->connection_timeout );
            $this->errstr  = $errstr;
        ...
        function _lining_array( $array,  $key_preffix = '', &$lined_array) {
            foreach ( $array as $key=>$value ) {
                $lineKey = $key_preffix? $key_preffix. '[' . $key . ']' : $key;
                if ( !is_array( $value ) )
                    $lined_array[$lineKey] = $value;
                else
                    $this->_lining_array( $value,  $lineKey, $lined_array);
            }
        ...
        function _merge_form_data( $param_array,  $param_name = '' ) {
            $params = '';
            $lined_array = array();
            $this->_lining_array( $param_array,  '', $lined_array);
            
            $format = ($param_name !=''?'&'.$param_name.'[%s]=%s':'&%s=%s');
            foreach ( $lined_array as $key=>$value ) {
                ...
                if ( $this->_response->get_header( 'Content-Length' ) != null ) {
                    ...
                    if ( $this->_response->get_header( 'Transfer-Encoding' ) != null ) {
                        if ( strtolower( $this->_response->get_header( 'Transfer-Encoding' ) ) == 'chunked' ) {
                            $body_temp = '';
                            while ( !feof( $this->_socket ) ) {
                                $body_temp .= fread( $this->_socket, 4096 );
                            }
                            $pos = 0;
                            do {
                                $chunk_size = '';
                                do {
                                    $cb = 1;
                                    $byte = substr($body_temp, $pos, $cb);
                                    $pos += $cb;
                                    $chunk_size .= $byte;
                                } while (preg_match('/[a-zA-Z0-9]/', $byte)); // read chunksize including \r
                                $pos++; // readtrailing \n
                                $chunk_size = hexdec($chunk_size);
                                $this_chunk = substr($body_temp, $pos, $chunk_size);
                                $pos += $chunk_size;
                                $body .= $this_chunk;
